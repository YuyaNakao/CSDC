...
// 制作者：松田
public class Player4_Score : MonoBehaviour
{
    ...
    [SerializeField]
    private int p4_num_score;      // 合計スコア

    [SerializeField]
    private int P4_score;   // プレイヤースコア
    ...
    void Start()
    {
        ...
        // スコア合計の算出
        p4_num_score = manager.score[0] + manager.score[1] + manager.score[2] + manager.score[3];
        P4_score = manager.score[3] / p4_num_score;

        // オブジェクトのスケールサイズが大きくなるので調整する
        // 最大移動量　＝　スコア＊１００倍　/ ４０
        player_max_move = P4_score * 100 / 40;
        //  player_max_move = manager.score[1] / 40;
        // 移動量の初期値
        move_score = 0;
        py = init_position;
